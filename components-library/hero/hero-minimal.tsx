...
export const HeroMinimal = ({
  ...
  trustedBy = {
    avatars: [
      { src: "https://avatars.githubusercontent.com/u/1?v=4", name: "Michael" },
      { src: "https://avatars.githubusercontent.com/u/2?v=4", name: "Tamara" },
      { src: "https://avatars.githubusercontent.com/u/3?v=4", name: "Diego" },
      { src: "https://avatars.githubusercontent.com/u/1?v=4", name: "Mark" },
      { src: "https://avatars.githubusercontent.com/u/2?v=4", name: "Tamang" },
    ],
    text: "Trusted by 400+ users",
    rating: "⭐️⭐️⭐️⭐️⭐️",
  },
      ...
      <section className="bg-white dark:bg-black text-black dark:text-white min-h-screen w-full flex flex-col justify-center items-center px-4 relative">
        ...
        {badgeText && (
          ...
          >
            ...
            >
              {badgeIcon || badgeText}
              {badgeIcon && <ArrowRight className="ml-2 h-4 w-4" />}
            </motion.div>
          </motion.div>
        )}
          ...
          >
            {primaryButton && (
              <button
                ...
                className="bg-black dark:bg-white text-white dark:text-black text-sm font-medium px-6 py-3 rounded-full hover:opacity-90 transition hover:cursor-pointer"
              >
                {primaryButton.text}
              </button>
            )}
            {secondaryButton && (
              <button
                ...
                className="text-black dark:text-white text-sm font-medium px-6 py-3 rounded-full border border-black/20 dark:border-white/20 hover:bg-black/5 dark:hover:bg-white/10 transition hover:cursor-pointer"
              >
        ...
        {trustedBy && (
          ...
          >
            {trustedBy.avatars && trustedBy.avatars.length > 0 && (
              <div className="flex items-center -space-x-2">
                {trustedBy.avatars.map((avatar, index) => (
                  <Avatar key={index} src={avatar.src} name={avatar.name} />
                ))}
              </div>
            )}
            {(trustedBy.rating || trustedBy.text) && (
              <div className="flex items-center gap-1 mt-2 sm:mt-0">
                {trustedBy.rating && <span>{trustedBy.rating}</span>}
                {trustedBy.text && <span>{trustedBy.text}</span>}
              </div>
            )}
          </motion.div>
        )}
