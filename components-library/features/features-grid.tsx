...
const Feature108 = ({
  ...
  const generatedTabs: Tab[] = featureItems && featureItems.length > 0
    ? featureItems.map((item, index) => ({
        value: `tab-${index + 1}`,
        icon: React.createElement(FEATURE_ICONS[index % FEATURE_ICONS.length], { className: "h-auto w-4 shrink-0" }),
        label: item.title,
        content: {
          badge: "Feature",
          title: item.title,
  ...
  const activeTabs = tabs || generatedTabs;
  ...
  return (
    <section className="py-32">
      <div className="container mx-auto">
        ...
        <Tabs defaultValue={activeTabs[0].value} className="mt-8">
